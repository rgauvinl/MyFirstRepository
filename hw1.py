...
def last_4_doubled(string):
    ...
    """
    # -4 starts at 4th character from end, to the end
    substr = string[-4:]

    # Returns 2 copies of substr
    return substr * 2
...
def has_789(nums):
    ...
    """
    # Loops through list, stopping 2 before end to mitigate index out of range
    for i in range(len(nums) - 2):
        if nums[i] == 7 and nums[i+1] == 8 and nums[i+2] == 9:
            # If sequence found, return True
            return True
        else:
            # If not found, continue loop
            continue
    # If loop completes without sequence, return False
    return False
...
def count_x_o(string):
    ...
    """
    count = 0
    # Loops through string, stopping 2 before end to mitigate index out of range
    for i in range(len(string) - 2):
        # Only increments count if 'x' is found in the first position and 'o' in the third, while ignoring the middle position
        if string[i] == 'x' and string [i+2] == 'o':
            count += 1
    # Returns total count of "x_o" found, after loop completes
    return count
...
def samesunmoon(string):
    ...
    *** This can be simplified using a Python string method ***
    """
    # Uses the count method on original string to easily return the number of occurrences of both 'sun' and 'moon' to compare
...
def trimmed_average(nums):
    ...
    """
    nums_sorted = sorted(nums) # Sorts nums from low to high
    # pop(0) removes lowest value, pop(-1) removes highest value
    nums_sorted.pop(0)
    nums_sorted.pop(-1)
    total = 0
    # Adds all elements and uses floor division on the length of the sorted list to return the average
    for i in nums_sorted:
        total += i
    return total // len(nums_sorted)
